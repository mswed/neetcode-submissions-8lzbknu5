...
class Solution:
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        while top <= bottom:
            ...
            if row_end == target:
                # We found the target at the end of a row
                return True
            if row_end > target:
                # The target can be on this row or on a previous one
                # Check if it's on this one
                if row_start == target:
                    # We found the target at the start of the row
                    return True
                if row_start > target:
